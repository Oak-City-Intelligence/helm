...
const buildPrompt = (it) => [
  ...
  '',
  'ITEM: ' + it.id + ' → target repo ' + (it.github || '(local-only — no hosted repo)') + ', base ' + it.base + ', work branch ' + it.branch + '.',
  '',
  'PROCEDURE:',
...
const reviewPrompt = (it, build, lens) => {
  ...
  return [
    ...
    ...focus,
    '',
    'ITEM: ' + it.id + '. Plan: ' + it.plan + '. Target repo: ' + (it.github || '(local-only — no hosted repo)') + '.',
...
const fixPrompt = (it, build, findings) => [
  ...
  '',
  'ITEM: ' + it.id + '. Plan: ' + it.plan + '. Worktree: ' + (build.worktree_path || '(from config worktree_root)') + '. Target repo: ' + (it.github || '(local-only — no hosted repo)') + '.',
  '',
...
const prPrompt = (it, build, reviews) => [
  ...
  '',
  'ITEM: ' + it.id + '. Target repo: ' + (it.github || '(local-only — no hosted repo)') + '. Base: ' + it.base + '. Branch: ' + build.branch + '.',
  'The branch is ALREADY pushed by the build agent — do not search the filesystem for the repo. Build worktree: ' + (build.worktree_path || '(see config worktree_root)') + '.',
  ...
  '',
  'OPEN THE PR in the mode the config defines — exactly one arm applies:',
  '  GITHUB MODE (the config names an owner/name repo): gh pr create --repo ' + (it.github || '(local-only — no hosted repo)') + ' --base ' + it.base + ' --head ' + build.branch + ' --title "<the item title>" --body "<what changed + verify summary + a GAUNTLET line: both reviewers passed, with their focus — NO AI attribution>". Capture the PR URL as pr_url.',
  '  BRANCH MODE (the config names no owner/name — self-hosted forge or no PR surface): never touch gh. The branch is already pushed. Open the PR through the forge\'s own API: read the config at ' + it.config + ' and use the forge API + token path it names. If the config names no forge, or the API errors: the item is still delivered by the pushed branch — return pr_url empty and say exactly what was unavailable in notes. Never invent a URL.',
  'NOTE: if this is a security fix on a PUBLIC repo, the PR body states WHAT was hardened, NOT the exploit mechanics (DECISIONS.md earned lesson).',
  '',
  'DONE condition: in GitHub mode the PR is open; in branch mode the reviewed branch is pushed, with a PR URL if the forge provided one.',
  '',
  'Build the one-line LEDGER entry in the harness format (<ISO-ts> | ' + it.id + ' | done | <note, incl. \'gauntlet: 2 reviewers passed\'> | <pr-url-or-branch>). Return it in ledger_line — do not write it to disk.',
  '',
  'Return PR_SCHEMA: status "done" only if the mode\'s done-condition is met (PR open in GitHub mode; branch pushed in branch mode); else "failed" with notes.',
].join('\n')
