...
7. COMMIT under the project identity. **NO AI ATTRIBUTION** — no \`Co-Authored-By: Claude\`, no "Generated with Claude Code", no 🤖 footer (harness §5). Commit = subject + body + the config's trailer, nothing else — read \`commit_trailer\` from the config: absent → \`helm: ${it.id}\`; the literal \`none\` → NO trailer (set that for public repos).
