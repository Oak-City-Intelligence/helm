...
ITEM: ${it.id} → target repo ${it.github}, base branch ${it.base}, work branch ${it.branch}.
...
5. COMMIT under the project identity. **NO AI ATTRIBUTION** — no \`Co-Authored-By: Claude\`, no "Generated with Claude Code", no 🤖 footer (harness §5). Commit = subject + body + \`helm: ${it.id}\` trailer, nothing else.
6. PUSH branch ${it.branch}, then open the PR:
...
7. Build the one-line LEDGER entry in the EXACT format from the harness (\`<ISO-ts> | <item-id> | <status> | <note> | <pr-url-or-branch>\`). Do not write it to disk — return it.

Return the structured result. status "done" only if the PR is open and verify passed. If you blocked, status "blocked" + block_question (pr_url empty). If verify failed unrecoverably, status "failed" + notes. The structured return IS your report — write no prose elsewhere.`
